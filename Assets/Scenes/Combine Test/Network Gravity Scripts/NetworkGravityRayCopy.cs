...
public class NetworkGravityRayCopy : NetworkBehaviour
{
    public GravityHandler gravityHandler;
    private PathRecorderAndReturnCopy pathRecorderAndReturnCopy;
    ...
    void ToIPosClientRpc(int hitObjectRegisterID)
    {
        ...
        foreach (GameObject clientObject in allClientObjects)
        {
            ...
            if (clientNetworkTransform != null)
            {
                ...
                if (hitObjectRegisterID == clientObjectRegisterID)
                {
                    pathRecorderAndReturnCopy = clientObject.GetComponent<PathRecorderAndReturnCopy>();
                    pathRecorderAndReturnCopy.StartRecord();
                }
            }
        }
    }
